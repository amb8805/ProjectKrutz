  ...
  public boolean sendBroadcast(Intent paramIntent)
  {
    ...
    synchronized (this.mReceivers)
    {
      ...
      if (j >= localArrayList1.size())
        break label534;
      LocalBroadcastManager.ReceiverRecord localReceiverRecord = (LocalBroadcastManager.ReceiverRecord)localArrayList1.get(j);
      if (i != 0)
    ...
    while (true)
    {
      ...
      {
        ((LocalBroadcastManager.ReceiverRecord)localArrayList2.get(k)).broadcasting = false;
        k++;
      }
      else
      {
        this.mPendingBroadcasts.add(new LocalBroadcastManager.BroadcastRecord(paramIntent, localArrayList2));
